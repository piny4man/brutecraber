// SHA3-512 OpenCL kernel for brutecraber GPU acceleration
// Keccak sponge construction. Rate = 72 bytes, output = 64 bytes.
// Supports single-block messages only (word length < 72 bytes).
...
__kernel void sha3_512_crack(
    ...
    // SHA3-512 rate = 72 bytes; single-block limit
    if (len >= 72) {
    ...
    // Multi-rate padding: 0x80 at byte position (rate - 1) = 71
    S[71 >> 3] ^= (ulong)0x80 << ((71 & 7) * 8);

    // Apply Keccak-f[1600]
    keccak_f_512(S);

    // Extract 64-byte digest from state (little-endian)
    uchar digest[64];
    for (int w = 0; w < 8; w++) {
        ulong val = S[w];
